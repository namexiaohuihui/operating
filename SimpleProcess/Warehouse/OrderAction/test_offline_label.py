...
class TestOfflineLabel(unittest.TestCase):
    ...
    def test_offline_time_default(self):
        """时间下拉框中默认显示的文字"""
    ...
    def test_offline_time_input(self):
        """时间输入框中默认显示的文字"""
        label_option = self.op_br.get_ele_text_vlue("reservationtime", 'id', 'value')
        assert label_option == '今日', '时间输入框中默认显示的文字判断有误:%s' % label_option
        del label_option
        print("关键字默认检验完毕")
        pass

    def test_offline_status_default(self):
        """关键字下拉框的默认值显示"""
    ...
    def test_offline_key_default(self):
        """关键字下拉框的默认值显示"""
    ...
    def test_offline_shopname(self):
        """点击店铺名称进行跳转"""
    ...
    def test_offline_single(self):
        """点击页面线下订单按钮弹出弹窗"""
        self.op_br.is_visible_clicks("button.btn.btn-xs.btn-search.J_ipt.modal-btn", "css")
        label_text = self.op_br.get_ele_text_vlue("myModalLabel", 'id')
        assert label_text == '生成订单', '点击线下订单弹窗标题判断有误:%s' % label_text
        pass

    def test_deficiency_module(self):
        """转预约+更换配送员+关闭调度+翻页等功能没有做"""
        assert False, "订单操作没有做没有做"
