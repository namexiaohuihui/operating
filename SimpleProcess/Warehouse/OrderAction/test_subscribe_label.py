...
@software: PyCharm
@file:  test_subscribe_label.py
@time: 2018/12/23 15:54
@Software: PyCharm
@Site    : 
@desc:
"""
...
class TestSubscribelabel(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.muen_int = 2
        cls.module_int = 2
    ...
    def test_subscribe_time_default(self):
        """时间下拉框中默认显示的文字"""
    ...
    def test_subscribe_time_input(self):
        """时间输入框中默认显示的文字"""
        label_option = self.op_br.get_ele_text_vlue("reservationtime", 'id', 'value')
        assert label_option == '全部', '调度顶订单关键字下拉默认值不对:%s' % label_option
        del label_option
        print("关键字默认检验完毕")
        pass

    def test_subscribe_key_default(self):
        """关键字下拉框的默认值显示"""
    ...
    def test_subscribe_orderid(self):
        """点击订单ID进行跳转"""
    ...
    def test_subscribe_shopname(self):
        """点击店铺名称进行跳转"""
