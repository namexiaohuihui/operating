...
@software: PyCharm
@file:  test_distribution_label.py
...
class TestDistributionLabel(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.muen_int = 2
        cls.module_int = 4
    ...
    def test_distribution_time_default(self):
        """时间下拉框中默认显示的文字"""
        elucidate = self.work.get_option_text("select[id='type']")
        assert elucidate == '下单时间', '时间下拉框中默认显示的文字判断有误:%s' % elucidate
        del elucidate
        pass

    def test_distribution_time_input(self):
        """时间输入框中默认显示的文字"""
        label_option = self.op_br.get_ele_text_vlue("reservationtime", 'id', 'value')
        assert label_option == '今日', '时间输入框中默认显示的文字默认值不对:%s' % label_option
        del label_option
        print("关键字默认检验完毕")
        pass

    def test_distribution_status_default(self):
        """关键字下拉框的默认值显示"""
        label_option = self.work.get_option_text("select[name='status']")
        assert label_option == '订单状态', '调度顶订单关键字下拉默认值不对:%s' % label_option
        del label_option
        print("关键字默认检验完毕")
        pass

    def test_distribution_key_default(self):
        """关键字下拉框的默认值显示"""
    ...
    def test_distribution_orderid(self):
        """点击订单ID进行跳转"""
    ...
    def test_distribution_shopname(self):
        """点击店铺名称进行跳转"""
    ...
    def test_distribution_disp(self):
        """点击配送员名字"""
        attribute = self.op_br.is_visible_clicks("tbody > tr:nth-child(1) > td:nth-child(4) > a", "css")
        assert attribute, "界面没有收获地址可点击"
        label_text = self.op_br.get_ele_text_vlue("ul.breadcrumb>li", 'css')
        assert label_text == '配送员管理', '点击配送员名字跳转出现错误:%s' % label_text
        self.op_br.driver.back()
        pass

    def test_distribution_address(self):
        """点击收货地址进行跳转"""
        attribute = self.op_br.is_visible_clicks("tbody > tr:nth-child(1) > td:nth-child(5) > a", "css")
        assert attribute, "界面没有收获地址可点击"
        label_text = self.op_br.get_ele_text_vlue("ul.breadcrumb>li", 'css')
